...
def exploratory_random (B,G,e_m,mapping_edges_indexes, list_edges,num_trials = 10,cover_factor =1):
  """
  one_rand_timer = exploratory_random (B,G,e_m,mapping_edges_indexes, list_edges,num_trials = 10)
  """
  start = timer()
...
def max_trials_random (B,G,e_m,mapping_edges_indexes, list_edges,num_trials):
  """
  B, Best_couple = max_trials_random (B,G,e_m,mapping_edges_indexes, list_edges,num_trials)
  """
  Best_couple = G,G
  for i in range(num_trials):
    R = rand_bin_array(k, m) #R0
...
def pruning_three(G,e_m, print_statement = False,wall_time = 600):
  ...
  if print_statement == True:
      print("")
      print ("all edges: ", len(all_edges_list))
      #print (all_edges_list)

      print ("must_have_edges: ", len(must_have_edges))
      #print (must_have_edges)
      #print ("n-len(must_have_edges): %s, num_missing_edges: %s" %(len(all_edges_list)-len(must_have_edges), num_missing_edges))
  l = n_choose_k(len(all_edges_list)-len(must_have_edges), num_missing_edges) #number of combinations
  set_limit = 5*(10**5)
  ...
  if set_strategy == "exact_result":
      ...
      # random
      one_rand_timer = exploratory_random (B_upper, G,e_m,mapping_edges_indexes, list_edges,num_trials = 10)
      cover_factor = 1
      ...
      if num_trials > 0:
          ...
          B_upper, Best_couple = max_trials_random (B_upper,G,e_m,mapping_edges_indexes, list_edges,num_trials)
          ...
          new_edges  = sum_with_mirror_edges(list_edges,e_m) # sum of each edge and its mirror
          bigG= nx.Graph()
          bigG.add_weighted_edges_from(new_edges)
          one_spanning_tree= find_MST(bigG, print_stat = False)
          B_T = 0
          B_mT = 0
          mirror_MST_edges = []
          for edge in (one_spanning_tree.edges()):
              #print (edge)
              if edge[0]< edge[1]:
                  u,v= edge[0], edge[1]
              else:
                  u,v= edge[1],edge[0]
              w = (G[u][v]['weight'])
              #print ("G[u][v]['weight']: ",w)
              u_m,v_m,w_m = e_m[(u,v,w)]
              mirror_MST_edges.append((u_m,v_m,w_m))
              B_T +=w
              B_mT +=w_m
          one_mirror_tree = nx.Graph()
          one_mirror_tree.add_weighted_edges_from(mirror_MST_edges)
...
def find_MST(bigG,edges = [],print_stat = True):
  """
  ...
  elif (not len(edges) >0) and (not nx.is_empty(bigG)):
    edges = bigG.edges.data()
    new_edges = flatten_edges_with_weights(edges)
  else:
    new_edges = []
...
"""## num spanning tree of a complete graph

def num_st_complete(G):
"""
...
this theorem is used to calculate the NUMBER of spanning tree of a connected graph

* Build an adjacency matrix
* Replace the diagonal entries with the degrees of the corresponding nodes
* Replace all the other ones excluding the one's included in the
diagonal by −1
* Then find the minor of any one of the elements to get your answer.

# https://cs.stackexchange.com/questions/119863/kirchhoffs-spanning-tree-algorithm
"""
def kirch_st(G):
  """
  tot_num = kirch_st(G)
  ...
  #adjacency matrix
  adiac = nx.adjacency_matrix(G)
  adiac = (np.array(adiac.todense().data).reshape(size,size))
  A = np.where(adiac<1,0,1) # kick out the weights... only 1s and 0s
  #diagonal matrix
  ...
  #newL = Matrix(np.array(L.data).reshape(size,size))# convert to np array
  newL = Matrix(L)
  #print (newL)
  result = newL.adjugate()[0,0] #find cofactor matrix (determinant of every size-1,size-1 square)
  return int(result)

"""## total number of spanning trees"""

def tot_num_spanning_trees(G, print_statement = False):
  """
  result = tot_num_spanning_trees(G, print_statement = False)
  the method for connected graph applied on complete graph returns the same result.
  the method for the complete graph is much quicker, hence the differenciation here
  """
  if is_graph_complete(G):
    start = timer()
    result = num_st_complete(G)
    #if print_statement == True: # same number it takes just much longer to compute if n is big
      #print (result,kirch_st(G))
    end = timer()
  else:
    start = timer()
    result = kirch_st(G) #using method 3
    end = timer()


  if print_statement == True:
    print ("total number of existing spanning trees is: ", result)
    print ("time required for computation : ",end - start)
  return result
...
#or
B_upper,subgraph,subgraph_mirror, delta_time = pruning_three(G,e_m, print_statement = True, wall_time=60)
