# -*- coding: utf-8 -*-
"""assignment_clean.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/10jkrzGS0oF819jmQuH-v0VRe4xkqZhJI
"""
...
def pruning(list_edges,list_nodes, e_m, E_weights_list, print_statement = False,wall_time = 600):
  ...
  one_spanning_tree, one_mirror_tree = all_edges,all_edges # extreme case that a graph is already a spanning tree
  ...
  if l > set_limit:
        ...
        for i in range(l):
          ...
          # for adopting eventually a different strategy
  ...
  if set_strategy == "exact_result":
      ...
      sorted_list_tuples_all_weights = sorted(all_sums.items())

      ## find the min sum of weight over all spanning trees found
      for min_sum in set(sorted_min):
        selected_indexes_of_spanning_trees = list(key for key, val in sorted_list_tuples_all_weights if val <= B_upper)

        if len(selected_indexes_of_spanning_trees) == 0:
          break
        ...
        if min_sum >= B_upper: #there arent any more spanning trees with B less than B_upper
          ...
          all_trees_min_sum = [selected_spanning_trees[index] for index,val in sorted_list_tuples_all_weights if val == min_sum]
          ...
          for selected_edges in all_trees_min_sum:
            ...
            if sum_T_mirror <= min_sum: #solution found
              ...
              if B_upper > sum_T_mirror: # > min_sum
      ...
      if num_trials > 0:
          ...
          one_spanning_tree= find_MST(new_edges, print_stat = False)
          B_T = 0
          B_mT = 0
          one_mirror_tree = []
  ...
  return B_upper, one_spanning_tree, one_mirror_tree, delta_time,set_strategy
...
if (myarr[0].isdigit() == False) or (myarr[1].isdigit() == False):
        sys.stdout.write("NO\n")
        sys.exit()
...
for row in myarr[2:]: # only the edges here
  ...
  #line_list = x.split(' ')
  #if all(i.isdigit() for i in line_list) == False:
          #sys.stdout.write("NO\n")
          #sys.exit()

  #string2float_list = list(map(float,row.split(' ')))
  try:
    stringfloat_list = list(map(float,row.split(' ')))
  except ValueError:
      sys.stdout.write("NO\n")
      sys.exit()

  string2int_list = [round(i) for i in stringfloat_list]
  if len(string2int_list) != 3:
      sys.stdout.write("NO\n")
      sys.exit()

  u,v,w = (string2int_list[0],string2int_list[1],string2int_list[2])
  mapping_edges_indexes[edge_idx] = (u,v,w) # eg e_1 = (1,2,42)
...
is_connected(neightbours, n)
if is_connected(neightbours, n) == False:
    sys.stdout.write("NO\n")
    sys.exit()

B_upper,one_spanning_tree, one_mirror_tree, delta_time,set_strategy = pruning(list_edges,list_nodes, e_m, E_weights_list, print_statement = False,wall_time = 600)

# check if out of MST
if set_strategy == "MST of sum of edges":
    solution_edges =[]
    for edge in one_spanning_tree:
        u,v,_ = edge
        if u< v:
            ...
            w = edges_dict[(v,u)]
        solution_edges.append((u,v,w))
else:
    solution_edges = one_spanning_tree


solution_edges = sorted(solution_edges, key=lambda x: mapping_output[x])
